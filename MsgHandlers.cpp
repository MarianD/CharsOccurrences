...
//
void TabCtrl_OnSize(HWND hwndTabCtrl, UINT state, int cx, int cy)
{
    RECT     rect, * pRect  = &rect;
    Status * pStatus        = (Status *) GetProp(hwndTabCtrl, cn::PointerToStatus);
    WNDPROC  oldTabCtrlProc = (WNDPROC) (pStatus->getOldTabCtrlWndProc());

    FORWARD_WM_SIZE(hwndTabCtrl, state, cx, cy, oldTabCtrlProc);  // Particularly for displaying arrows for short tabs row
